...
import { createClient, deleteClient, updateClient } from "./clients.api";
import { useAllClients, useClients } from "./clients.hooks";
...
export default function ClientsPage() {
  ...
  const { clients, total, totalPages, loading, error, setError, reload } = useClients(page, search);
  const { allClients, reload: reloadAllClients } = useAllClients();
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      closeModal();
      await Promise.all([reload(), reloadAllClients()]);
    } catch (err) {
